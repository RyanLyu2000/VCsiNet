...
class Cost2100channel(nn.Module):
    ...
    def forward(self, input, mask_BCWH, uplinkChannel, avg_pwr=None):
        ...
        channel_in = channel_in[:, 0:L:2] + channel_in[:, 1:L:2] * 1j   # 复数信号调制
        ...
        channel_out = self.cost2100_forward(channel_in, uplinkChannel)
        ...
        if avg_pwr:
            ...
            return channel_rx * torch.sqrt(pwr)

    def cost2100_forward(self, channel_in, uplinkChannel):
        ...
        sigma = torch.sqrt(mean_power / (2 * 10 ** (self.chan_param / 10))).reshape(-1, 1, 1)               # [batch_size, 1]
