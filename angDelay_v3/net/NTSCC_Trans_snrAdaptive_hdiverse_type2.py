...
from csi_flatFramework.angDelay_v3.layer.analysis_transform import AnalysisTransform, Mlp
from csi_flatFramework.angDelay_v3.layer.synthesis_transform import SynthesisTransform
...
from csi_flatFramework.angDelay_v3.layer.jscc_encoder_etaAttention_type2 import JSCCEncoder, SNRAdaptiveJSCCEncoder
from csi_flatFramework.angDelay_v3.layer.jscc_decoder_etaAttention_type2 import JSCCDecoder, SNRAdaptiveJSCCDecoder
from csi_flatFramework.angDelay_v3.utils.utils import BCHW2BLN, BLN2BCHW
from csi_flatFramework.angDelay_v3.channel.channel import Channel
from csi_flatFramework.angDelay_v3.channel.cost2100channel_inSnrAdaptive import Cost2100channel
from csi_flatFramework.angDelay_v3.statics import evaluator
...
class NTC_net(nn.Module):
    def __init__(self, config):
        ...
        self.fe = SNRAdaptiveJSCCEncoder(**config.fe_kwargs)
        self.fd = SNRAdaptiveJSCCDecoder(**config.fd_kwargs)
    ...
    def forward(self, input_image, Hu, eta_batch, snr_batch):
        B, C, H, W = input_image.shape
        ...
        # forward NTSCC_fefd
        s_masked, mask_BCHW, indexes = self.fe(y, y_likelihoods.detach(), hy, eta=eta_batch, snr=snr_batch)
        avg_pwr = torch.sum(s_masked ** 2) / mask_BCHW.sum()
        if self.channel_type == 'awgn':
            s_hat = self.channel.forward(s_masked, avg_pwr) * mask_BCHW
            y_ntscc_hat = self.fd(s_hat, indexes, eta=eta_batch, snr=snr_batch)
        else:
            s_hat, patches_snr = self.channel.forward(s_masked, mask_BCHW, Hu, avg_pwr, snr=snr_batch)   # 需要修改信道
            y_ntscc_hat = self.fd(s_hat, indexes, eta=eta_batch, snr=patches_snr)
        ...
        return mse_ntc, bpp_y, None, mse_ntscc, cbr_y, x_hat_ntc, x_hat_ntscc, nmse_ntc, nmse_ntscc, mse_y

    def forward_ntscc(self, input_image, Hu):
        B, C, H, W = input_image.shape
        ...
        if self.channel_type == 'awgn':
            ...
            s_hat, _ = self.channel.forward(s_masked, mask_BCHW, Hu, avg_pwr)  # 需要修改信道
        # indexes
        ...
        # cbr_y = mask_BCHW.sum() / (B * H * W * 3 * 2)
        cbr_y = mask_BCHW.sum() / (B * H * W * 2 * 2)  # csi只有2个通道
        return bpp_y, bpp_z, mse_ntscc, cbr_y, x_hat_ntscc, nmse_ntscc, mse_y
