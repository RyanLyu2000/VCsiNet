...
class NTC_net(nn.Module):
    ...
    def forward(self, input_image, Hu, eta_batch, snr_batch):
        ...
        # forward NTSCC_fefd
        s_masked, mask_BCHW, indexes = self.fe(y, y_likelihoods.detach(), hy, eta=eta_batch)
        avg_pwr = torch.sum(s_masked ** 2) / mask_BCHW.sum()
        if self.channel_type == 'awgn':
            s_hat = self.channel.forward(s_masked, avg_pwr) * mask_BCHW
        else:
            s_hat, _ = self.channel.forward(s_masked, mask_BCHW, Hu, avg_pwr, snr=snr_batch)  # 需要修改信道
        # indexes
        y_ntscc_hat = self.fd(s_hat, indexes, eta=eta_batch)
    ...
    def forward_ntscc(self, input_image, Hu):
        ...
        if self.channel_type == 'awgn':
            ...
            s_hat, _ = self.channel.forward(s_masked, mask_BCHW, Hu, avg_pwr) * mask_BCHW  # 需要修改信道
        # indexes
