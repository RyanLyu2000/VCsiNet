from angDelay_v3.layer.layers import *
import torch
...
class BasicLayer(nn.Module):
    def __init__(self, dim, out_dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, patch_embed=False,
                 SNR_choice=None, eta_choice=None):

        super().__init__()
        self.dim = dim
        if downsample is not None or patch_embed:
...
class AnalysisTransform(nn.Module):
    def __init__(self, img_size=(256, 256), in_chans=3,
        ...
        self.S = self.in_channels * img_size[0] // (2 ** self.num_layers)
        self.patch_embed = PatchEmbed(img_size, 2, self.in_channels, embed_dims[0], nn.LayerNorm)       # (32 * 32)  stride=2, channel=2, => embed_dims[0] * 16, nn.LayerNorm
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dims[i_layer - 1]) if i_layer != 0 else embed_dims[0],
                               ...
                               input_resolution=(self.in_channels * self.patches_resolution[0] // (2 ** (i_layer))),       # 16 => 8 => 4
                               ...
                               patch_embed=True if i_layer == 0 else False,
    ...
    def forward(self, x, SNR=None, eta=None):
        ...
        x = self.norm(x)
        B, L, N = x.shape
        x = x.reshape(B, self.S, N).permute(0, 2, 1)
        return x
    ...
    def flops(self):
        ...
        for i, layer in enumerate(self.layers):
            flops += layer.flops()
        flops += self.num_features * self.patches_resolution[0] // (2 ** self.num_layers)
        # flops += self.num_features * self.num_classes
        return flops

    def update_resolution(self, H, W):
        self.patches_resolution = (H, W)
        self.S = H // (2 ** self.num_layers)
        for i_layer, layer in enumerate(self.layers):
...
if __name__ == '__main__':
    ...
    from csi_newFramework.angDelay_v2.utils.utils import *
