...
from angDelay_v3.utils.h_relu import hrelu
...
class SynthesisTransform(nn.Module):
    def __init__(self, img_size, out_chans=3,
        ...
        self.H = img_size[0]
        self.W = img_size[1]
        self.patches_resolution = (self.out_channels * img_size[0] // 2 ** len(depths))
        ...
        self.outconv = nn.Conv2d(self.out_channels * embed_dims[-1] // self.H, self.out_channels, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
    ...
    def forward(self, x, SNR=None, eta=None, out_conv=True):
        ...
        for i_layer, layer in enumerate(self.layers):
            if SNR is not None:
                ...
                x = layer(x)
        B, L, N = x.shape                                           # B, S, C => 200, 32, 256 => 200, 32, 32, 8
        x = x.reshape(B, self.out_channels, self.H, N // self.W, self.W).permute(0, 1, 3, 2, 4).reshape(B, -1, self.H, self.W)
        if out_conv:
            x = self.outconv(x)
            x = self.out_relu(x)
        return x
